...
The main characteristic of good data science is: 

> Good data science is distinguished from bad data science primarily by a repeatable, thoughtful, skeptical application of an analytic process to data in order to arrive at supportable conclusions. (Jeff Leek)


In these sessions we will explore one of the main tools for data analysis, _the R statistical program_,  and a package or set of utilities, specific to datascience, the _tidyverse package_

One might be tempted to start analyzing a dataset using complex statistical models that are then difficult to interpret because one is not familiar with the data. Also, there is a great interest in the result of a particular statistical test or level of significance rather than in the evaluation of the process that allowed a certain test to be performed. This makes many researchers delegate the task of analysis to a statistician, without being familiar with the data. 

![](../Figs/stats-pipeline.jpg)
...
An exploratory data analysis focus on searching for discoveries, trends, correlations, or relationships between the measurements of multiple variables to generate ideas or hypotheses. 
...
> Exploratory data analysis is for myself, inferential is to convince others
...
*   __Tabular data__ is a table of values, each associated with a variable and an observation. Tabular data is __tidy__ if each value is placed in its own cell, each variable in its own column, and each observation in its own row. 
    ...
    So far, all of the data that you've seen has been tidy. In real-life, most data isn't tidy, so we'll come back to these ideas again in __Data Wrangling__.
